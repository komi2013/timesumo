...
class OthersController extends Controller {
    ...
    public function index(Request $request,$directory,$controller,$action,
        ...
        foreach ($obj as $d) {
            $day35[substr($d->time_start,0,10)][$d->schedule_id] = [$d->public_title,$d->tag,$url];
            $start = new Carbon($d->time_start);
            $end = new Carbon($d->time_end);
            while ($start->diffInDays($end) > 0) {
                $start->addDay();
                $day35[$start->format('Y-m-d')][$d->schedule_id] = [$d->public_title,$d->tag,$url];
            }
        }
        if ($_SERVER['SERVER_NAME'] == 'timebook.quigen.info') {
            $obj_sync = DB::table('t_sync')->where('usr_id',$target)->get();
        ...
        return view('calendar.others', compact('day35','today','prev','next','url'));
