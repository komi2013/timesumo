...
<table style="width:100%;" v-for="(d,k) in book">
    ...
    <tr><th>店</th><td>{{d['group_name']}}</td></tr>
    <tr>
        <td style="height:50px;"><input type="radio" :id="k+'_r_1'" value="1" v-model="d['review_to_salon']"><label :for="k+'_r_1'" >悪い</label></td>
        <td style="height:50px;"><input type="radio" :id="k+'_r_5'" value="5" v-model="d['review_to_salon']"><label :for="k+'_r_5'">良い</label></td>
    </tr>
    <tr><td colspan="2"><textarea class="column1" v-model="d['usr_comment']">{{d['usr_comment']}}</textarea></td></tr>
</table>
...
var content = new Vue({
  ...
  methods: {
    update: function (e) {
        ...
        $.post('/User/ReviewUpdate/',param,function(){},"json")
        .always(function(res){
            if(res[0] == 1){
                location.href = '';
            }else{
